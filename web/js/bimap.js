...
BiCollection.prototype = {
    put: function(key, value) {
        var kString = key.toString();
        var vString = value.toString();

        // if already contains this value - remove previous key
        if (this.values.hasOwnProperty(vString)) {
            var prevKey = this.biMap.val(vString);

            delete this.keys[prevKey];
            delete this.biMap.kv[prevKey];
        }
      
        // if already contains this key - remove previous value
        if (this.keys.hasOwnProperty(kString)) {
            var prevValue = this.biMap.key(kString);

            delete this.values[prevValue];
            delete this.biMap.vk[prevValue];
        }

        this.keys[kString] = key;
        this.values[vString] = value;
        this.biMap.set(kString, vString);
    },

    getValue: function(key) {
        var vString = this.biMap.key(key.toString());

        if (vString !== undefined) {
            return this.values[vString];
        }
    },

    getKey: function(value) {
        var kString = this.biMap.val(value.toString());

        if (kString !== undefined) {
            return this.keys[kString];
        }
    }
};